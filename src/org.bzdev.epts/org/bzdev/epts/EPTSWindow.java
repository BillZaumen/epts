...
class OffsetPane extends JPanel {
    ...
    */
	...
	for (String name: ptmodel.getVisiblePathVariableNames()) {
	    pathnames.add(name);
	}
...
public class EPTSWindow {
    ...
    */
	...
	continuePathMenuItem.setEnabled(canAddToBezier());
	...
	if (extendingPath) {
	    // Need to terminate the path after deleting newly added
	    // points.
    ...
    JMenuItem undoMenuItem;	// for undo operations.
    JMenuItem saveMenuItem;
    JMenuItem saveAsMenuItem;
    JMenuItem addToPathMenuItem; // for Edit menu.
    JMenuItem continuePathMenuItem; // for Edit menu.
    ...
    */
	...
	String oldPathName = null;
	if (selectedRow == -1) {
	    Set<String> vnameSet = ptmodel.getVisiblePathVariableNames();
	    if (vnameSet.isEmpty()) return;
	    String[] vnames =
		vnameSet.toArray(new String[vnameSet.size()]);
	    String vname;
    ...
    JMenu filterMenu = new JMenu(localeString("Filters"));
	...
	menuItem.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    ...
		    int n = ptmodel.getRowCount();
		    if (n > 0) {
			Enum rmd = ptmodel.getRowMode(--n);
			if (rmd != null  && rmd instanceof EPTS.Mode) {
			    switch((EPTS.Mode)rmd) {
			    case PATH_END:
			    case LOCATION:
				// We only want to undo while entering points.
				// Otherwise the behavior is unpredictable to
				// users.
				return;
			    default:
				break;
	...
	menuItem.setEnabled(false);
	ActionListener appendToBezierAL = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    // select a path.
		    int index = -1;
		    clearLSR();
	...
	menuItem.addActionListener(appendToBezierAL);
	editMenu.add(menuItem);

	menuItem = new JMenuItem(localeString("ExtendBezier"),
				 vk("VK_ExtendBezier"));
	menuItem.setAccelerator(KeyStroke.getKeyStroke
				(KeyEvent.VK_E, InputEvent.ALT_DOWN_MASK));
	continuePathMenuItem = menuItem;
	menuItem.setEnabled(false);
	menuItem.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    appendToBezierAL.actionPerformed(e);
		    if (extendingPath) {
			extendPathIndex = extendPathStart;
		    }
		}
	    });
	editMenu.add(menuItem);
			...
			if (state instanceof SplinePathBuilder.CPointType) {
				...
				if (ttable != null) {
					...
					addToPathMenuItem.setEnabled
					    (canAddToBezier());
					continuePathMenuItem.setEnabled
					    (canAddToBezier());
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (locState) {
				...
				*/
				...
				ptmodel.insertRow(insertRowIndex, "",
						  insertMode,
						  x, y, xp, yp);
				insertRowIndex = -1;
	...
	ptfilters = new PTFilters(frame, filterMenu, ptmodel, () -> {
		return terminatePathOps();});
	menuItem = new JMenuItem(localeString("newFilter"),
				 vk("VK_newFilter"));
	menuItem.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    if (!terminatePathOps()) return;
		    // terminatePathOps(true);
		    /*
		    if (selectedRow != -1) {
			selectedRowCleanup();
		    }
	...
	*/
	...
	contPathMenuItem = new JMenuItem(localeString("ContPath"));
	contPathMenuItem.setEnabled(true);
	contPathMenuItem.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    clearLSR();
		    cancelPathOps();
		    onExtendPath();
	...
	popupMenu.add(contPathMenuItem);
	...
	if (nextState != null && e.isPopupTrigger()) {
		...
		*/
	    ...
	    extendPathMenuItem.setEnabled(!hasLoop(selectedRow));
	    contPathMenuItem.setEnabled(!hasLoop(selectedRow));
	    Enum mode = ptmodel.getRow(selectedRow).getMode();
	    if (mode == EPTS.Mode.LOCATION) {
		changeMenuItem.setEnabled(false);
		deleteMenuItem.setEnabled(true);
		extendPathMenuItem.setEnabled(false);
		contPathMenuItem.setEnabled(false);
    ...
    */
    ...
    // For selectedRow != -1 when handling VK_ESCAPE
    // put also used to clean up if a filter is used when a row
    // is selected.
    private void selectedRowCleanup() {
	if (moveLocOrPath) {
	    ptmodel.moveObject(selectedRow,
			       lastXSelected,
			       lastYSelected,
    ...
    private boolean terminatePathOps() {
	clearLSR();
	if (insertingArc) {
	    insertingArc = false;
	    resetState();
	...
	*/
	    int result =
		JOptionPane.showConfirmDialog(frame, frameText, frameTitle,
					      JOptionPane.OK_CANCEL_OPTION,
					      JOptionPane.QUESTION_MESSAGE);
			...
			if (selectedRow != -1) {
				...
				if (insertRowIndex != -1) {
				    ...
				    */
				    ...
				    selectedClosedPath = false;
				    addToPathMenuItem
					.setEnabled(canAddToBezier());
				    continuePathMenuItem
					.setEnabled(canAddToBezier());
				    // resetState();
				} else if (moveLocOrPath) {
				    ptmodel.moveObject(selectedRow,
						       x, y, xp, yp, true);
				} else {
				    lastSelectedRowMoved =
